...
|---|---|---|---|
| 1 | QR Code Generator | 🔒 **CLAIMED (2026-08-12)** — Cover the roster code sheet with a test | The roster-labelled code sheet shipped without a suite. The sharp edge is that generated lines must be tab-separated: a roster full of "Ruiz, Marisol" fed through the comma delimiter splits the label off the content. Worth pinning, along with the Avery auto-switch not overriding a layout already chosen. |
| 2 | Gallery Walk QR Codes | 🔒 **CLAIMED (2026-08-12)** — Projector rotation display | A fullscreen station-and-timer view so the rotation clock is readable across the room, not only at the laptop. |
| 3 | Testing Accommodations Reference Card Generator | 🔒 **CLAIMED (2026-08-12)** — Filter the grid by accommodation | Show only students with a chosen accommodation, for planning testing-day room assignments. |
| 4 | Staff Directory / Quick-Reference Builder | 🔒 **CLAIMED (2026-08-12)** — Group by department | Sub-headers grouping the table and print view by department instead of one flat alphabetical list. |
| 5 | Fitness & Skill Assessment Tracker | 🔒 **CLAIMED (2026-08-12)** — Sortable results columns | Click an event header to sort the grid fastest-to-slowest or highest-to-lowest, for finding outliers in a projected view. |
| 6 | Exit Ticket / Bell Ringer Generator | 🔒 **CLAIMED (2026-08-12)** — Quarter-inch grid response area | A fourth response style beside ruled lines, blank box and plain space, for the maths and science prompts that want a sketch on a grid. Needs `print-color-adjust: exact`, since browsers drop background images from print by default. |
| 7 | Silent Reading (SSR) Log Tracker | Class reading-diet panel | Now that books carry genres: one line per genre with how many students have read at least one, sorted fewest-first. Ordered by popularity it says what the class already likes; ordered by scarcity it says what to book-talk on Monday. Pair it with a line naming who has read none of a filtered genre. |
| 8 | Parent/Guardian Contact Log | Reason tally above the history | A breakdown of whatever is currently filtered, with the positive count called out separately. "How many of my contacts home were good news" is the number a teacher is quietly keeping score of, and a log that only ever records problems is exactly what this would make visible. |
| 9 | Number Talks / Mental Math Routine Board | Leave the answers off the session record | The printed record carries every computed answer, which is right for the teacher's copy and wrong for the one that goes home with the student who was pulled out. A checkbox, on by default, that removes them and says on the page that it did. |
| 10 | Grade Distribution Visualizer | Say so when the chart copy fails | Clipboard image writes need a secure context and a permission that can be refused. The copy button currently has no failure state, so a blocked copy looks identical to a successful one — it should flash "Use Download PNG" and point at the button beside it. |
| 11 | Quiz / Review Game Board | Storage usage readout | Clue images are the first thing here that can realistically fill localStorage. A running "saved boards are using about N MB" line under the editor, warning past roughly 70% of the cap, turns the full-storage failure from a surprise into something a teacher can see coming. |
...
| 61 | Quiz / Review Game Board | Audio in a clue | Record or attach a short clip — a pronunciation, a music excerpt, a primary-source recording — stored in IndexedDB rather than inline, with a play control on the projected clue. |
