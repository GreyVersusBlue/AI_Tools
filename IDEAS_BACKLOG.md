...
|---|---|---|---|
| 1 | Parent/Guardian Contact Log | Contact reason tags | A second axis — attendance, grades, behavior, positive — alongside method, filterable in the history table and included in the existing export. |
| 2 | Rubric Builder | Peer-review print format | A fifth print mode that reduces the rubric to a peer feedback form — criteria plus a comment line each, no point columns. |
| 3 | Sub Binder / Day Bundle Generator | Sub feedback slip page | Append a printable one-page form the substitute fills in before leaving: what got done, who helped, what went wrong. |
| 4 | Graph Paper & Number Line Generator | Printed calibration ruler | A test page with an inch/cm scale so a teacher confirms once per printer that true-to-scale output actually printed true. |
| 5 | Grade Distribution Visualizer | Copy chart to clipboard | Write the rendered chart to the clipboard as an image so it drops straight into a PLC document with no download step. |
| 6 | East Middle Schedule Browser | Personal notes overlay | Let the viewing teacher attach their own per-period notes — duty, meeting, plan — stored locally and printed with their schedule. |
| 7 | Command Center | Seating chart panel | A registry panel rendering the current period's chart read-only from `seating-chart-v1`, sized for the projector. |
