...
| 8 | Digital Escape Room / Puzzle Lock Builder | Answer matching for contractions | `normalizeTextAnswer` strips apostrophes without collapsing the word, so a student typing "it's a keyboard" against an accepted "a keyboard" is told they're wrong — the comment in `lock.html` already claims this case works. |
