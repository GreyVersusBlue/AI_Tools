...
- **Roster loading (P2).** A "Load a saved Name Picker roster…" dropdown +
  ...
  read-only, same pattern `group-team-generator.html` already uses. This
...
- **`pe-tournament-stations.html` overlap (P7) — left alone as instructed.**
  ...
  reuses whatever `pe-tournament-stations.html` has. Worth a deliberate
...
- A real second-display presentation mode if a teacher ever asks for the
  ...
  browser tab.
- The P7 engine-sharing question with `pe-tournament-stations.html`, now
