...
- **Partially done — lightweight bridge shipped, full hub deferred.**
  ...
  *(A full shared hub was explicitly out of scope for this round. Instead,
  this tool and `vocab-flashcard-generator.html` each got a small read-only
  bridge to the other's saved lists, copying the pattern
  `writing-prompt-generator.html`'s `wpg-rubric-link.js` established: no
...
- **Resolved 2026-08-10 — partially, with a lightweight answer.** What shape
  ...
  `vocab-flashcard-generator.html`'s own storage format (which gained
  ...
  `roleplay-scenario-generator.html`'s vocabulary log; any of
