...
- Some tools have real test suites (`Tools/*/test/`) — schedule,
  seating-chart, name-picker, image-to-pdf. Run them when touching those
  tools. **Known broken as of 2026-08-11:** `Tools/board-check/` is missing
  from the repo (not on disk, not tracked by git), so the schedule suite can't
  import its harness and `sync-social-tags.mjs` above can't be run at all; and
  the image-to-pdf suite hardcodes a Linux sandbox (Playwright at
  `/opt/node22/lib/node_modules`, Chromium at `/opt/pw-browsers/chromium`), so
  it won't run on Windows. `final-grade-checker` has no `test/` folder at all,
  despite older notes claiming otherwise. Until that's fixed, verify in a real
  browser against a local static server instead, and say so in the PR.
