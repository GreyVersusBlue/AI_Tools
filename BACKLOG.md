...
**Last shipped.** Rank 1 — **Path 6 P3's fourth increment: six more builders, and the first that
...
link cannot move onto the handoff table until the page loads the sheet. **So the next P3 increment
is 018, 019, 048 and 077 — none of which is a copy** (two are promised to Path 12 P2, two hold a
`student: true` key), which means the cheap half of P3 is finished and every row left needs a
decision written down before any wiring.
...
|---:|---|---|---|---|---|
| 1 | Path 6 P3 — extend sharing to the builders that do not share yet. **Four increments shipped: #237 (v172) 052, 057, 070, 073, 079, 081, single-document; #239 (v173) 047, 065, 072, named-library; #244 (v175) 041, 051, 069, 082, 083, also named-library; #246 (v176) 049, 058, 074, 075, 076, 078, the rest of the single-document group that was a copy.** The [Path 6](#path-6--share-everywhere) section lists every remaining generator/builder grouped by storage shape. **The cheap half is done and every row left needs a decision written down before any wiring:** **018** and **019** belong to Path 12 P2 (rank 40) — share them there or accept that their payload changes shape when the question bank lands; **048** and **077** each hold a `student: true` key, so each needs 073's per-field split applied to a product question (what an accommodations card is *without* the student is the sharper of the two); **046** is rank 2's first job, because its hand-built `?timeline=` link cannot move onto the handoff table until the page loads the sheet; **038 was mis-filed here and has been moved to the bank-plus-settings group** — it keeps device settings plus a name→pasted-text map whose own source comment says the pasted data can contain student data; the twelve bank-plus-settings ones (014, 016, 023, 025, 029, 053, 055, 061, 062, 066, 067, 071) are a design question per tool; and **045 still goes last or waits** for ranks 29–30. Add a row to `Tools/share/test/smoke-share-rollout.mjs`'s table rather than a per-tool suite, and a whole section when a tool's arrival is a third kind (075's merge is 5c) | site | 2+ | `llfkj1` 2026-09-11 13:03 UTC | [Path 6](#path-6--share-everywhere) |
...
adopter. **P3 is four increments in and still open: #237 (v172) took 052, 057, 070, 073,
