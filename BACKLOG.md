...
*Current as of `main` after PR #244, 2026-09-08. Rewrite this header when your phase
merges — that is step 6 of the definition of done, and it is not optional.*

**Last shipped.** Rank 1 — **Path 6 P3's third increment: five more named-library builders**
...
**Start here: rank 1 — Path 6 P3, and it is the same row #237 just took an increment out of.
It is a 2+ row, so it is the whole batch on its own** — do not pair it with anything, do one honest
increment, ship it, and leave the row in place with its Item text rewritten again. Rank 2 is P4, a 1,
so it is a batch of one when the list reaches it.

**What is left of P3, and it is not what the phase text originally listed.** Six of the eleven named
tools are done (052, 057, 070, 073, 079, 081) and **044 was never P3's — it adopted in P2 (#233)**,
which is worth knowing because the phase's list has now been one out for two increments. **Left:
047, 065, 072 and 045**, plus every "generator" with a saved configuration, which **nobody has
enumerated** — doing that enumeration once and writing the list into the Path 6 section is itself
worth half an hour, because "plus every generator" is how a phase list goes stale.

**047, 065 and 072 are one coherent batch and are easier than the six just shipped, not harder.**
All three keep a **named library** — `LIST_KEY` + `DATA_PREFIX` + `CURRENT_KEY` — so an arrival is
saved *beside* what is there under a suffixed name, exactly as every P1/P2 adopter does, and **none
of #237's confirm machinery applies**: no `load()` return value, no `confirm()`, no "kept the local
copy" sentence. Copy 054 or 003, not 052. **Take 045 last or not at all yet**: it compiles other
tools' storage rather than owning much of its own, and Path 10 P2/P3 (ranks 28 and 29) are about to
re-base it on section providers, so sharing it now means shipping a payload whose shape is already
scheduled to change.

**And add a row to `Tools/share/test/smoke-share-rollout.mjs`'s table rather than writing a fourth
per-tool share suite** — that is what the rollout shape is for. One caution from writing it: a
fixture that names a record **id** is asserting that nothing remaps ids, which is false on any tool
that merges an arrival into local records.
...
|---:|---|---|---|---|---|
| 1 | Path 6 P3 — extend sharing to the builders that do not share yet. **Increment 1 shipped (#237, v172): 052, 057, 070, 073, 079, 081, all single-document. Increment 2 shipped (#239, v173): 047, 065, 072, all named-library. Increment 3 shipped (#244, v175): 041, 051, 069, 082, 083, also named-library — a straight copy of #239, which is the finding.** The [Path 6](#path-6--share-everywhere) section lists every remaining generator/builder grouped by storage shape, which is what decides how much of #237 applies. **Two named-library ones are left and neither is a copy:** **048** holds `apl_portfolio_v1`, which the registry marks `student: true`, so it needs 073's per-field split decided first — what an art portfolio label is *without* the student is a design question, not wiring; and **019** is one of the four already promised to Path 12 P2 (rank 40), so share it there or accept that its payload changes shape when the question bank lands. **So the next increment is the ten single-document ones — 018, 038, 046, 049, 058, 074, 075, 076, 077, 078 — which are #237's shape and bring back the `confirm()`-when-`load()`-found-something rule** (023 and 077 also carry a `student: true` key; 018 is Path 12 P2's). The twelve bank-plus-settings ones (014, 016, 023, 025, 029, 053, 055, 061, 062, 066, 067, 071) are a design question per tool, not wiring, and should not be batched with either. **045 still goes last or waits:** it compiles other tools' storage and ranks 29–30 (Path 10 P2/P3) are about to re-base it on section providers. Add a row to `Tools/share/test/smoke-share-rollout.mjs`'s table rather than a per-tool suite | site | 2+ | `ibj6p7` 2026-09-08 19:04 UTC | [Path 6](#path-6--share-everywhere) |
...
adopter. **P3 is three increments in and still open: #237 (v172) took 052, 057, 070, 073,
079 and 081 — the single-document builders — #239 (v173) took 047, 065 and 072, the
named-library ones, and #244 (v175) took five more of those (041, 051, 069, 082, 083),
taking `share.js` and `qr-draw.js` to 29 of 86 and `state-link.js` to 30.** **P2 is finished**, in three increments all on 2026-09-08: #231 (v169) took 028,
...
- **P3 — Extend to the builders that don't share yet. Three increments shipped
  ...
  **047, 065 and 072 are done** (#239) — the named-library set (`LIST_KEY` +
  ...
  below applies. **Left: 045, and the generators — the enumeration this row kept
  ...
  **30 have "generator", "builder", "maker" or "creator" in the name and write to
  ...
  - **Named library (a list key + a per-name data prefix + a current pointer) — the same
    ...
    registry parameters, and not one of #239's three decisions needed changing. **Two are
    left and neither is a copy:** **048** (art portfolio labels) holds a `student: true` key
    and needs 073's per-field split decided first, and **019** (escape rooms) is promised to
    Path 12 P2.
  - **One document or one settings blob — #237's shape, so the `confirm()`-when-`load()`-
    found-something rule applies as written:** **018**, **038**, **046**, **049**, **058**,
    **074**, **075**, **076**, **077**, **078**.
  - **A built-in bank plus custom additions plus device settings, where what a teacher would
    ...
    **014**, **016**, **023**, **025**, **029**, **053**, **055**, **061**, **062**, **066**,
    **067**, **071**. Deciding what travels here is a real design question per tool, not
    wiring, which is why they should not be batched with the first group.

  **Three carry a `student: true` key and need 073's per-field split before they can share
  anything:** 023 (`gvb-exit-ticket:tally`), 048 (`apl_portfolio_v1`) and 077
