...
*Current as of the repo-review PR, 2026-09-23. **Keep this section under ~80 lines.** It is
...
**Last shipped — the repo-review follow-ups (`CACHE_VERSION` v180).** Not a ranked row; asked
for directly by Devon after a whole-repo review.
- `_shared/sw-register.js` now asks for **persistent storage** (`navigator.storage.persist()`),
  which nothing on the site did before. It asks only once the origin holds data, never on a
  fullscreen or busy page, never from `file://`. Covered by `npm run test:storage-persist`.
- A new guard, **`npm run check:inline-sinks`**, is a shrink-only ratchet on the dynamic markup
  sinks (`innerHTML` and similar) in the inline scripts of the **50** pages that take link input.
  ESLint does not see inline script, and a share link is input from outside the room.
- **Registry student-data audit.** All **191** keys and prefixes that were unmarked were read
  by what each tool actually stores. **Five** now carry `student: true`: 003's rubric scores,
  085's last letter's values, 024's strategy library, 084's seminar documents and 043's
  field-trip slips. **Seven more mix student names into teacher content** and are deliberately
  left unmarked, because 009's rollover deletes whole keys and promises to keep templates.
  They are now part of rank 6, starting with 044's `medicalAlerts`. `HISTORY.md` has the
  per-key verdicts.
- **Not done:** deleting rank 89's four dead trees was blocked by the session's sandbox
  permissions. The row stays, and it is still a one-line `git rm`.
- Before it: **#252** (v179), Path 6 P3's seventh increment — 014, 023, 025, 067 and 071 on the
  share sheet. Three keys found holding student names and marked. Detail in `HISTORY.md`.

**Start here: rank 1 — Path 6 P3.** It is a 2+ row, so it is a batch on its own. **Three
bank-plus-settings tools are left: 016, 029, 038**, then 046 (rank 2's first job) and 045
(waits for Path 10 P2/P3), and the row closes.
- **029** is 019-shaped: `loadFromParams()` treats *any* query parameter as a prefill, so exclude
  the share parameter from it. It sits on `_shared/theme.css`, so the sheet needs 018's scoped
  token block. What it shares is the **presets**.
- **038** needs a decision written down first: a *chart's settings* travel, but a pasted dataset
  (which can hold student data) only if the teacher picks it. The sheet has no per-dataset row yet.
- **016** is 2,826 lines with a checkout inventory. Read `load*()` for all four keys first.
- The rules for a P3 increment are at the end of the Path 6 section: add a row to the rollout
  suite rather than a new suite; check link size when images are involved; grep the tool's own
  `innerHTML` sinks (`npm run check:inline-sinks -- --list <tool>` prints them).
...
- **Interleave per-tool improvements with platform work?** Standing decisions say "keep platform
  first", so every teacher-visible per-tool idea sits at rank 96 or below behind 95 platform rows.
...
**Numbers (2026-09-23; re-measure, do not carry forward):**

| Fact | Value |
|---|---|
| `CACHE_VERSION` | `v180` — `check:precache -- --base origin/main` is the thing to trust |
...
| Inline markup sinks | **399** across the 50 pages that take link input (`check:inline-sinks` baseline) |
...
| Tier 1 rows | **176**, contiguous; per-tool rows start at rank **95** |
...
|---:|---|---|---|---|---|
| 1 | Path 6 P3 — extend sharing to the builders that do not share yet. **Seven increments shipped: #237 (v172) 052, 057, 070, 073, 079, 081, single-document; #239 (v173) 047, 065, 072, named-library; #244 (v175) 041, 051, 069, 082, 083, also named-library; #246 (v176) 049, 058, 074, 075, 076, 078, the rest of the single-document group; #248 (v177) 018, 019, 048, 077, the four that each needed a decision written down first; #250 (v178) 053, 055, 061, 062, 066, the first five bank-plus-settings generators; #252 (v179) 014, 023, 025, 067, 071, five more.** The [Path 6](#path-6--share-everywhere) section lists every remaining generator/builder grouped by storage shape. **Two are waiting on something:** **046** is rank 2's first job, because its hand-built `?timeline=` link cannot move onto the handoff table until the page loads the sheet; and **045** compiles other tools' storage and Path 10 P2/P3 are about to re-base it, so share it after those or accept that its payload changes shape. **So what is left of this row is three bank-plus-settings tools — 016, 029, 038 — each surveyed in #252's handoff:** 029 already has a `?field=` prefill link of its own that treats *any* parameter as a prefill (exclude the share parameter from `loadFromParams()`) and sits on `theme.css` (018's scoped token block), and shares its **presets**; 038 keeps a name→pasted-text map of datasets that may hold student data, and needs the per-dataset decision written down before wiring; 016 is 2,826 lines with a checkout inventory — read `load*()` for all four keys first. **Ask each one the three questions:** what is the *vocabulary* (#248); what is the **subtraction** that must stay, because an arrival that does not ask may only add (#250); and **what does this page write with `innerHTML`** (#250, 066). Add a row to `Tools/share/test/smoke-share-rollout.mjs`'s table rather than a per-tool suite — a library in a new layout takes `libraryHooks`, and what lands outside the merged key is asserted through `afterMerge` (#252) — and a whole section when a tool's arrival is a new kind | site | 2+ | | [Path 6](#path-6--share-everywhere) |
...
adopter. **P3 is seven increments in and still open — #252 (v179) took 014, 023, 025, 067 and 071,
